...
map< string, ll > mauser, maproduct;
int main()
{
   freopen("Reviews_Binary.csv", "r", stdin);
   freopen("In_Recommender_Format.csv", "w", stdout);

   ll i, j, col, pos, lineno, valuser = 1, valproduct = 1;
   string st, tuplee, stri[10];
   ...
   rep(lineno, 1, 564687)
   {
      ...
      rep(j, 0, 3)
      {
         if( j==0 && !maproduct[ stri[j] ] )
         {
            maproduct[ stri[j] ] = valproduct++;
            stri[j] = con_ll_to_str( maproduct[ stri[j] ] );
         }
         else if( j==0 )
         {
            stri[j] = con_ll_to_str( maproduct[ stri[j] ] );
         }
         else if( j==1 && !mauser[ stri[j] ] )
         {
            mauser[ stri[j] ] = valuser++;
            stri[j] = con_ll_to_str( mauser[ stri[j] ] );
         }
         else if( j==1 )
         {
            stri[j] = con_ll_to_str( mauser[ stri[j] ] );
